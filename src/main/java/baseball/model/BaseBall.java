...
public class BaseBall {
	...
	/**
	 * 투구번호 배열에서 <code>number</code>가 어느 인덱스에 있는지 확인합니다.
	 *
	...
	/**
	 * 중복된 숫자가 있는지 중복을 제거 전후의 크기를 비교합니다.
	 *
	 * @param textBaseBall 문자열로 이루어진 투구번호
	 * @throws IllegalBaseBallArgumentException 중복된 숫자가 있을 경우 발생합니다.
	 */
	 ...
	 * @return 포함 하고 있으면 True 없으면 false
	 */
	...
	/**
	 * <code>number</code>의 값을 찾을 때 까지 <code>start</code>의 값을 +1 하여 재귀적으로 실행합니다.
	 * <code>start</code> 의 값이 <code>end</code> 의 값보다 큰 경우 <code>baseBall</code> 배열의 존재하지
	 * 않는다는 것으로 간주하여 -1 을 반환합니다.
	 *
	 * @param baseBall 투구번호가 들어가 있는 int 배열
	 * @param number   확인하고자 하는 숫자
